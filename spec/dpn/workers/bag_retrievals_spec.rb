...
describe DPN::Workers::BagRetrievals do
  ...
  # The replication data is irrelevant to the bag retrieval worker, use a dummy
  ...
  describe '#perform' do
    let!(:bag_retrieve) { DPN::Workers::BagRetrieve.new replication }
    before do
      allow(DPN::Workers::BagRetrieve).to receive(:new).and_return(bag_retrieve)
    end
    it 'returns true for success' do
      allow(bag_retrieve).to receive(:transfer).and_return(true)
      result = replication_worker.perform
      expect(result).to be true
    end
    it 'returns false for failure' do
      allow(bag_retrieve).to receive(:transfer).and_return(false)
      result = replication_worker.perform
      expect(result).to be false
    end
    it 'raises exceptions' do
      allow(bag_retrieve).to receive(:transfer).and_raise('failed')
  ...
  describe '#replications_query' do
    ...
    it 'query[:store_requested] is false' do
      expect(query).to include(:store_requested)
      expect(query[:store_requested]).to be false
    end
